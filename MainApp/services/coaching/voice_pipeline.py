    ...
    def process_event(self, event, exercise, metrics):
        ...
        try:
            ...
            if status_code == 429 or "429" in error_text or "RESOURCE_EXHAUSTED" in error_text:
                self.disabled_until = now + 60
                self.error_message = "Gemini quota reached. AI coaching is paused for 60 seconds."
            else:
                self.disabled_until = now + 15
                self.error_message = "AI coaching is temporarily unavailable. Workout tracking continues."
            return None
